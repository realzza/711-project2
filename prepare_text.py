...
if __name__ == '__main__':
    ...
    for pdf in all_pdfs:
        ...
        for i in tqdm(range(n_pages), desc="[%s]"%(pdf_name)):
            ...
            tokenized_page = [nlp(line) for line in ' '.join(curr_text.encode("ascii", "ignore").decode('utf-8').split('\n')).split('. ')]
            pdf_page_sentence = [{'text':(' '.join([w.text for w in line])).replace(' - ','[SSUUBB]').replace('- ','').replace('[SSUUBB]',' - ').replace(' %','%').replace(' ne - ', ' fine - ').replace(' nd', ' find')+' . ','paperid':pdf_name} for line in tokenized_page]
